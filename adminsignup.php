...
if (isset($_POST['register']) && $_POST['register'] == 'register' && isset($_POST['roleid'])) {
    ...
    // if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($confirmpassword)) {
         ...
         //validate role field
         if (empty($roleid)) {
            $regerror['roleid'] = "<span class='text text-danger'>Select a role!</span>";
        }
    ...
    // if all error variables are empty then create an object i.e if there is no validation error
        ...
        if (count($errorlist)== 0) {
            ...
            header("Location: http://localhost/devpro/dashboard.php");
...
<?php 
    ...
    <form method="post" action="<?php  echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
        ...
        ?>
        <div class="form-group row">
            <label for="" class="col-form-label col-md-3">Roles</label>
            <select class="form-control col-md-10" name="roleid" id="">
                <option value="">Choose Role</option>   
                <?php 
                    foreach ($roles as $key => $rolearray):
                        $role_id = $rolearray['role_id'];
                        $role_title = $rolearray['role_title'];
                        if ($_POST['role_id'] == $role_id) {  
